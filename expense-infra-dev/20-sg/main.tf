module "mysql_sg" {
    source = "git::https://github.com/rameshpalli345/terraform-vpc-sg.git?ref=main"
    project_name = var.project_name
    environment = var.environment
    sg_name = "mysql"
    vpc_id = local.vpc_id
    common_tags = var.common_tags
    sg_tags = var.sql_sg_tags
}
...
resource "aws_security_group_rule" "backed_app_alb" {
  type              = "ingress"
  from_port         = 8080
  to_port           = 8080
  protocol          = "tcp"
